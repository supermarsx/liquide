...
| T-24 | **T**ampering | Crafted USB descriptors exploit kernel VHCI driver | Session ↔ USB (B3) | 3 | Low | Critical | Tier 3 only: `liquid-usb-broker` runs in isolated systemd service (DynamicUser, PrivateNetwork, seccomp-bpf). Broker validates descriptor length and endpoint count before forwarding to VHCI. Kernel USBIP module patched/updated. Device class blocklist rejects HID+mass-storage combo descriptors. See [spec-system.md §6.5a](spec-system.md). | Medium — requires Tier 3 + kernel bug |
| T-25 | **I**nformation Disclosure | Mass storage DLP exfiltration via redirected USB drive | Session ↔ USB (B3) | 3 | Medium | High | Policy-level device class blocklist (mass storage disabled by default). Per-device VID/PID allowlist. `usb.tier = 3` required. Audit log records all USB attach/detach with device serial, class, VID/PID. Optional read-only mount enforcement via broker policy (`usb.mass_storage_read_only = true`). | Low — policy dependent |
| T-26 | **D**enial of Service | USB device flood exhausts VHCI slots or broker resources | Session ↔ USB (B3) | 3 | Medium | Medium | Broker enforces max 8 concurrent devices per session (`usb.max_devices = 8`). Rate-limits attach requests (max 2/sec). cgroup memory+CPU limits on broker process. VHCI slot pool is pre-allocated and bounded. Excess attach requests rejected with `USB_ATTACH_DENIED`. | Low |
| T-27 | **E**levation of Privilege | Broker privilege escalation via VHCI kernel interface | Session ↔ USB (B3) | 3 | Very Low | Critical | `liquid-usb-broker` runs as DynamicUser (no persistent UID). seccomp-bpf allowlist restricts syscalls to read/write/ioctl/poll/epoll. AppArmor profile confines file access to `/dev/vhci*` and broker socket only. No `CAP_SYS_ADMIN`; uses pre-opened VHCI fd passed from systemd. PrivateNetwork prevents lateral movement. | Very Low — requires kernel + seccomp bypass |
| T-28 | **S**poofing | VID/PID spoofing to bypass USB device blocklist | Session ↔ USB (B3) | 3 | Medium | Medium | Broker cross-checks USB descriptor fields (bDeviceClass, bInterfaceClass) against declared VID/PID to detect mismatches. Known-bad VID/PID combos maintained in updatable blocklist (`usb.blocklist_url`). Anomaly detection: alert on devices claiming common VID (e.g., Logitech) with unexpected interface class. Audit log flags mismatches. | Low — compensating controls |
