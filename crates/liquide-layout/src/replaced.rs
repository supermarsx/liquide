...
const REPLACED_TAGS: &[&str] = &["img", "video", "canvas", "svg", "embed", "object", "iframe", "input"];
...
pub fn layout_replaced(
    ...
    let explicit_w = style.width.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h);
    let explicit_h = style.height.resolve_px(container_height, base_font_size, font_size, viewport_w, viewport_h);

    // Resolve padding
    let pad_top = style.padding.top.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
    let pad_right = style.padding.right.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
    let pad_bottom = style.padding.bottom.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
    let pad_left = style.padding.left.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
    ...
    let mar_top = style.margin.top.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
    let mar_bottom = style.margin.bottom.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
    ...
    let (used_w, used_h) = match (explicit_w, explicit_h) {
        ...
        (Some(w), None) => {
            let cw = match style.box_sizing {
                BoxSizing::BorderBox => (w - pad_left - pad_right - border_left - border_right).max(0.0),
        ...
        (None, Some(h)) => {
            let ch = match style.box_sizing {
                BoxSizing::BorderBox => (h - pad_top - pad_bottom - border_top - border_bottom).max(0.0),
    ...
    let (mar_left, mar_right) = if ml_auto && mr_auto {
        ...
        let mr = style.margin.right.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
        let remaining = (container_width - outer_w - mr).max(0.0);
        (remaining, mr)
    } else if mr_auto {
        let ml = style.margin.left.resolve_px(container_width, base_font_size, font_size, viewport_w, viewport_h).unwrap_or(0.0);
