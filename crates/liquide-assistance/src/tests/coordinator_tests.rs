...
fn make_observer() -> Observer {
    Observer::new("obs-1".into(), "Alice".into(), ObserverRole::HelpDesk, AssistanceMode::ViewOnly)
}
...
fn test_request_disabled() {
    ...
    cfg.enabled = false;
    let mut coord = AssistanceCoordinator::new(cfg, ModeConfig::default(), StealthConfig::default(), PermissionsConfig::default());
    let obs = make_observer();
...
fn test_join_with_code() {
    let mut coord = make_coordinator();
    let invite_msg = coord.create_invite("owner-1", AssistanceMode::Interactive, 300, 5).unwrap();
    let obs = make_observer();
...
fn test_end_assistance() {
    let mut coord = make_coordinator();
    let invite_msg = coord.create_invite("owner-1", AssistanceMode::ViewOnly, 300, 1).unwrap();
    let obs = make_observer();
...
fn test_drain_audit_events() {
    let mut coord = make_coordinator();
    let obs = make_observer();
    coord.request_assistance(&obs, "target-1", AssistanceMode::ViewOnly, "help").unwrap();
