...
fn make_security_admin() -> Observer {
    Observer::new("sec-1".into(), "SecAdmin".into(), ObserverRole::SecurityAdmin, AssistanceMode::Stealth)
}

fn make_helpdesk() -> Observer {
    Observer::new("hd-1".into(), "HelpDesk".into(), ObserverRole::HelpDesk, AssistanceMode::ViewOnly)
}
...
fn test_stealth_audit_interval() {
    let obs = make_security_admin();
    let cfg = StealthConfig { audit_interval_seconds: 5, ..StealthConfig::default() };
...
fn test_stealth_expiration() {
    let obs = make_security_admin();
    let cfg = StealthConfig { max_duration_minutes: 1, ..StealthConfig::default() };
    let mut session = StealthSession::new(&obs, "target-1".into(), cfg).unwrap();
