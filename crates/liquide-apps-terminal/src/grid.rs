...
impl Grid {
    ...
    pub fn scroll_down(&mut self, n: u32) {
        for _ in 0..n {
            if self.scroll_top <= self.scroll_bottom
                && (self.scroll_bottom as usize) < self.cells.len()
            {
    ...
    pub fn set_scroll_region(&mut self, top: u32, bottom: u32) {
        ...
        let b = if bottom == 0 {
            ...
            (bottom.saturating_sub(1)).min(self.rows.saturating_sub(1))
        };
        if t < b {
            self.scroll_top = t;
    ...
    pub fn erase_display_to_beginning(&mut self) {
        let r = self.cursor_row as usize;
        let c = self.cursor_col as usize;
        for row in &mut self.cells[..r] {
            for cell in row.iter_mut() {
                *cell = Cell::default();
            }
        }
        if r < self.cells.len() {
            let end = (c + 1).min(self.cells[r].len());
            for cell in &mut self.cells[r][..end] {
                *cell = Cell::default();
            }
        }
    }
