...
fn state_key_press_release() {
    ...
    let press = InputEvent::Keyboard(KeyEvent::new(
        KeyCode::A,
        KeyState::Pressed,
        Modifiers::new(),
        30,
...
fn state_touch_begin_end() {
    ...
    let begin = InputEvent::Touch(TouchEvent::new(
        TouchPhase::Begin,
        TouchPoint::new(1, 50.0, 50.0, 1.0),
        0,
    ));
    ...
    let end = InputEvent::Touch(TouchEvent::new(
        TouchPhase::End,
        TouchPoint::new(1, 50.0, 50.0, 0.0),
        1,
    ));
...
fn state_multiple_touches() {
    ...
    for i in 0..5 {
        let begin = InputEvent::Touch(TouchEvent::new(
            TouchPhase::Begin,
            TouchPoint::new(i, 10.0 * i as f32, 10.0, 1.0),
            0,
        ));
        state.handle_event(&begin);
    }
...
fn state_active_touch_count() {
    ...
    state.handle_event(&InputEvent::Touch(TouchEvent::new(
        TouchPhase::Begin,
        TouchPoint::new(2, 10.0, 10.0, 1.0),
        0,
    )));
    assert_eq!(state.active_touch_count(), 2);
    state.handle_event(&InputEvent::Touch(TouchEvent::new(
        TouchPhase::Cancel,
        TouchPoint::new(1, 0.0, 0.0, 0.0),
        0,
    )));
    assert_eq!(state.active_touch_count(), 1);
}
