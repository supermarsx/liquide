...
fn retries_before_next_codec() {
    let mut mgr = FallbackManager::new(
        FallbackConfig { enabled: true, max_retries: 2, alert_on_fallback: true },
        vec![HwEncoderApi::Vaapi],
    );

    let action = mgr.handle_failure(HwEncoderApi::Vaapi, CodecId::H264, FallbackReason::EncoderError);
...
fn exhausts_all_codecs_then_software() {
    let mut mgr = FallbackManager::new(
        FallbackConfig { enabled: true, max_retries: 0, alert_on_fallback: false },
        vec![HwEncoderApi::Vaapi],
    );

    // Fail all three codecs
    let _ = mgr.handle_failure(HwEncoderApi::Vaapi, CodecId::H264, FallbackReason::EncoderError);
    let _ = mgr.handle_failure(HwEncoderApi::Vaapi, CodecId::H265, FallbackReason::EncoderError);
    let action = mgr.handle_failure(HwEncoderApi::Vaapi, CodecId::Av1, FallbackReason::EncoderError);
...
fn disabled_fallback_gives_up() {
    let mut mgr = FallbackManager::new(
        FallbackConfig { enabled: false, max_retries: 3, alert_on_fallback: false },
        vec![HwEncoderApi::Nvenc],
    );
    let action = mgr.handle_failure(HwEncoderApi::Nvenc, CodecId::H264, FallbackReason::EncoderError);
