...
impl TimeZoneDatabase {
    ...
    pub fn new() -> Self {
        Self { zones: build_timezone_table() }
    }
    ...
    pub fn find(&self, id: &str) -> Option<&TimeZone> {
        ...
        self.zones.iter().find(|tz| tz.id.to_ascii_lowercase() == lower)
    }
    ...
    pub fn by_offset(&self, offset_minutes: i32) -> Vec<&TimeZone> {
        self.zones.iter().filter(|tz| tz.utc_offset_minutes == offset_minutes).collect()
    }
...
fn build_timezone_table() -> Vec<TimeZone> {
    vec![
        ...
        tz("America/Argentina/Buenos_Aires", "Buenos Aires", -180, "ART", false),

        // UTC-2 to UTC+0
        tz("Atlantic/South_Georgia", "South Georgia", -120, "GST", false),
