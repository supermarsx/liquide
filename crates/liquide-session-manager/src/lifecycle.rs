...
use crate::service::{RestartPolicy, ServiceId, ServiceState};
    ...
    pub fn tick(&mut self, registry: &mut ServiceRegistry) -> Vec<LifecycleEvent> {
            ...
            if let Some(entry) = registry.get_mut(&id) {
                ...
                let should_restart = match &entry.descriptor.restart_policy {
                    ...
                    RestartPolicy::OnFailure { max_retries, .. } => {
                        !clean_exit && entry.restart_count < *max_retries
                    }
                };
                ...
                if should_restart {
                    ...
                    self.restart_timers.insert(
                        id.clone(),
                        Instant::now() + Duration::from_millis(backoff_ms),
                    );
                } else {
                    entry.state = if clean_exit {
        ...
        for id in ready {
            ...
            if let Err(e) = self.start_service(&id, registry) {
                events.push(LifecycleEvent::RestartFailed {
                    id: id.clone(),
                    error: e.to_string(),
                });
            } else {
