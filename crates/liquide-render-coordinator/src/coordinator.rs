...
impl RenderCoordinator {
    ...
    pub async fn new(config: RenderConfig) -> Result<Self> {
        ...
        info!(
            "Initializing render coordinator with {} window threads",
            config.window_threads
        );
        ...
        match &task.kind {
            ...
            RenderTaskKind::Dock => {
                if let Some(thread) = &self.dock_thread {
                    ...
                    return Err(RenderError::InvalidConfig(
                        "Dock rendering not enabled".to_string(),
                    ));
                }
            }
            RenderTaskKind::StatusBar => {
                if let Some(thread) = &self.statusbar_thread {
                    ...
                    return Err(RenderError::InvalidConfig(
                        "Status bar rendering not enabled".to_string(),
                    ));
                }
            }
            RenderTaskKind::Background => {
                if let Some(thread) = &self.background_thread {
                    ...
                    return Err(RenderError::InvalidConfig(
                        "Background rendering not enabled".to_string(),
                    ));
                }
            }
            RenderTaskKind::Wallpaper { .. } => {
                if let Some(thread) = &self.wallpaper_thread {
                    ...
                    return Err(RenderError::InvalidConfig(
                        "Wallpaper rendering not enabled".to_string(),
                    ));
                }
            }
    ...
    pub async fn poll_outputs(&self) -> Result<Vec<RenderOutput>> {
        ...
        while let Some(output) = self.window_pool.try_recv_any()? {
            self.metrics
                .record_completion(output.duration, output.success);
            outputs.push(output);
        }
        ...
        while start.elapsed() < timeout {
            ...
            if !others.is_empty() {
                ...
                if pending.len() + others.len() <= MAX_PENDING {
                    ...
                    warn!(
                        "Pending outputs at capacity ({}), dropping {} outputs",
                        MAX_PENDING,
                        others.len()
                    );
                }
