...
impl ThreadedRenderCoordinator {
            ...
            match rx.recv() {
                Ok(msg) => {
                    match msg {
                        ...
                        CoordinatorMessage::WaitForTask { task_id, timeout, response } => {
    ...
    pub fn submit_task(&self, task: RenderTask) -> Result<u64> {
        let (tx, rx) = channel();
        self.tx.send(CoordinatorMessage::SubmitTask {
            task,
            response: tx,
    ...
    pub fn poll_outputs(&self) -> Result<Vec<RenderOutput>> {
        let (tx, rx) = channel();
        self.tx.send(CoordinatorMessage::PollOutputs {
            response: tx,
        }).map_err(|_| RenderError::ChannelSend("Coordinator thread died".to_string()))?;
    ...
    pub fn wait_for_task(&self, task_id: u64, timeout: Duration) -> Result<RenderOutput> {
        let (tx, rx) = channel();
        self.tx.send(CoordinatorMessage::WaitForTask {
            task_id,
            timeout,
            response: tx,
        }).map_err(|_| RenderError::ChannelSend("Coordinator thread died".to_string()))?;
    ...
    pub fn metrics(&self) -> Result<RenderMetrics> {
        let (tx, rx) = channel();
        self.tx.send(CoordinatorMessage::GetMetrics {
            response: tx,
        }).map_err(|_| RenderError::ChannelSend("Coordinator thread died".to_string()))?;
    ...
    pub fn render_window(&self, window_id: u64, is_focused: bool) -> Result<u64> {
        let task = RenderTask::new(
            0,
            crate::render_task::RenderTaskKind::Window { window_id, is_focused },
        );
