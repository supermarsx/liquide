...
fn store_set_and_get() {
    ...
    store
        .set(ClipboardFormat::PlainText, b"hello".to_vec(), 1, 0)
        .unwrap();
    assert_eq!(
        store.get(&ClipboardFormat::PlainText),
        Some(b"hello".as_ref())
    );
}
...
fn store_overwrite() {
    let mut store = ClipboardStore::new(1024);
    store
        .set(ClipboardFormat::PlainText, b"first".to_vec(), 1, 0)
        .unwrap();
    store
        .set(ClipboardFormat::PlainText, b"second".to_vec(), 2, 1)
        .unwrap();
    assert_eq!(
        store.get(&ClipboardFormat::PlainText),
...
fn store_clear() {
    let mut store = ClipboardStore::new(1024);
    store
        .set(ClipboardFormat::PlainText, b"data".to_vec(), 1, 0)
        .unwrap();
    store.clear();
    assert!(store.available_formats().is_empty());
    assert_eq!(store.total_bytes(), 0);
...
fn store_total_bytes() {
    ...
    store
        ...
        .unwrap();
    store
        .set(ClipboardFormat::Html, b"<b>hi</b>".to_vec(), 1, 0)
        .unwrap();
...
fn store_has_format() {
    ...
    store
        .set(ClipboardFormat::PlainText, b"x".to_vec(), 1, 0)
        .unwrap();
    assert!(store.has_format(&ClipboardFormat::PlainText));
}
...
fn store_owner() {
    let mut store = ClipboardStore::new(1024);
    store
        .set(ClipboardFormat::PlainText, b"x".to_vec(), 42, 0)
        .unwrap();
