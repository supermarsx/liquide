...
impl ClipboardOffer {
    ...
    #[must_use]
    pub fn new(source_id: u64, formats: Vec<ClipboardFormat>, timestamp_us: u64, serial: u64) -> Self {
        Self { source_id, formats, timestamp_us, serial }
    }
    ...
    pub fn preferred_text_format(&self) -> Option<&ClipboardFormat> {
        let priority = [ClipboardFormat::PlainText, ClipboardFormat::Html, ClipboardFormat::RichText];
        for p in &priority {
    ...
    pub fn preferred_image_format(&self) -> Option<&ClipboardFormat> {
        let priority = [ClipboardFormat::Png, ClipboardFormat::Jpeg, ClipboardFormat::Svg];
        for p in &priority {
            if let Some(f) = self.formats.iter().find(|f| *f == p) {
                return Some(f);
            }
